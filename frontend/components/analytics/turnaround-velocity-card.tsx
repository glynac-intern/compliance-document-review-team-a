...
export function TurnaroundVelocityCard({
  ...
  return (
    <div className="rounded-2xl border border-slate-200/90 bg-white p-5 sm:p-6 shadow-xs font-inter flex flex-col justify-between h-full">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4 p-3 rounded-xl bg-slate-50/70 border border-slate-100">
        <div>
          <span className="text-[10px] font-medium uppercase tracking-wider text-slate-400 block font-inter">
            SLA Adherence (24h)
          </span>
          <span className="text-lg sm:text-xl font-bold font-numbers tabular-nums text-slate-900 mt-0.5 block">
            {hasTurnaroundData && slaCompliancePct > 0 ? (
              `${slaCompliancePct.toFixed(1)}%`
            ) : (
              <span className="text-slate-400 font-normal text-sm">No data</span>
            )}
          </span>
        </div>
        <div>
              ...
              <div className="flex items-center gap-3 shrink-0 text-right">
                ...
                <span className="text-[12px] font-numbers font-medium tabular-nums w-14 text-right">
                  {hasDocs && tier.percentage > 0 ? (
                    <span className="text-[#1e4c77]">{tier.percentage.toFixed(1)}%</span>
                  ) : hasDocs ? (
